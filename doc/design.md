...
当我们计划使用并开始调研 Go 语言时，各种 Web 框架相关评测中 `fasthttp` 的优异表现让我们对 Go 有了很大的信心。但随着对 Go 的逐步深入学习和使用，当我们决定构建自己的 Web 框架时，还是选择了原生的 `net/http` 作为框架底层。

一方面是 1.7，1.8 版 Go 的 `net/http` 性能已经很好了，在我的 MBP 电脑上 Gear 框架与基于 `fasthttp` 的 Iris 框架（最快的）评测比分约为 `5:7`，已经不再是当初号称的10倍、20倍差距。如果算上应用的业务逻辑的消耗，这个差距会变得更小，甚至可以忽略。并且可以预见，随着 Go 版本升级优化，`net/http` 的性能表现会越来越好。
...
router 本身是个 `gear.Handler` 形式的中间件，但它的内含逻辑却又由更多的 `gear.Middleware` 类型的中间件组成。Gear 内置了一些核心的中间件，包括 `gear.Router` 中间件，`gear/logging` 目录下的 `logging.Logger` 中间件，`gear/middleware` 目录下的 `cors`, `favicon`, `secure`, `static` 中间件等，都是相同的组合逻辑。

另外 https://github.com/teambition 也有我们维护的一些 `gear-xxx` 的中间件，也非常欢迎开发者们参与 `gear-xxx` 中间件生态开发中来。

因此，中间件 `func(ctx *gear.Context) error` 是 Gear 组合扩展的元语。它有两个核心元素 `gear.Context` 和 `error`，其中 `gear.Context` 集成了 Gear 框架的核心开发能力（后面讲解），而返回值 `error` 会被框架层自动处理。

### 中间件处理流程

一个完整的 Request - Response 处理流程就是一系列中间件及其组合体的运行的流程，中间件按照引入的顺序逐一、单向运行（而非 **级联**），每个中间件解决一个特定的需求，与其它任何中间件没有耦合。
...
当请求是静态文件资源请求时，第二个响应 "Hello, Gear!" 的中间件就不在运行。

#### `error` 中断

当某一个中间件返回 `error` 时（比如 400 参数验证错误），中间件处理流程就被中断，后续的中间件不再运行，Gear 应用会自动处理这个错误，并做出对应的 response 响应（也能由开发者自定义错误响应结果）。开发者不再疲于 `error` 的处理，可以尽情的返回错误。

另外通过 `ctx.Error(err)` 或 `ctx.ErrorStatus(statusCode)` 主动响应错误的也算 `error` 中断。
...
上面这个示例代码包含了两种形式的 `error` 中断。无论哪种，其 err 都会被 Gear 框架层自动识别处理（后面详解），然后给客户端最佳响应。

与正常响应中断不同，`error` 中断及后面的异常中断都会导致通过 `ctx.After` 注入的 **after hooks** 逻辑被清理，不会运行（后面再详解）。

#### `context.Context` cancel 后中断

当中间件处理流还在运行，请求却因为某些原因被 `context.Context` 机制 cancel 时（如处理超时），中间件处理流程也会被中断，cancel 的 error 会被提取，然后按照类似 `error` 中断被框架自动处理。

#### `panic` 中断

最后就是某些中间件运行时可能出现的 panic error，它们能被框架捕获并按照类似 `error` 中断自动处理，错误信息中还会包含错误堆栈，以方便开发者跟踪错误。


## 3. 功能强大，完美集成 `context.Context` 的 `gear.Context`

`gear.Context` 是中间件 `func(ctx *gear.Context) error` 的另一个核心。它完全集成了 `context.Context`、`http.Request`、`http.ResponseWriter` 的能力，并且提供了很多核心、便捷的方法。开发者通过调用 `gear.Context` 即可快速实现各种 Web 业务逻辑。
...
Go 语言生态中，[Iris](https://github.com/kataras/iris)，[Gin](https://github.com/gin-gonic/gin) 等采用了 **级联** 流程控制模式。Gin 流程控制，其文档中的一个示例代码如下：
...
相对于 **单向顺序**，**级联** 唯一的优势就是在当前上下文中实现了 **after** 逻辑：在当前运行栈中，处理完所有后续中间件后再回来继续处理，正如上面 Logger。 Gear 框架使用 **after hooks** 来满足这个需求。

那么 **级联** 流程控制有什么问题呢？这里提出两点：

1. next 中的逻辑是个有状态的黑盒，当前中间件可能会与这个黑盒发生状态耦合，或者说这个黑盒导致当前中间件充满不确定性的状态，比如黑盒中是否出了错误？是否写入了响应数据？是否会 panic？这都是无法预知的。
